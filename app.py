...
def scrape_backup(origin, destination, start_date, end_date):
    ...
    for dest in destinations:
        for i in range(days):
            ...
            price = 200 + (hash(f"{origin}{dest}{date_str}") % 300)  # Random but consistent price
            
            sample_data.append({
                "origin": origin,
                "destination": dest,
                "depart_date": date_str,
                "return_date": (current_date + timedelta(days=7)).strftime("%Y-%m-%d"),
                "price": price,
                "airline": "DEMO",
...
def results():
    ...
    destination_airport = get_airport_by_code(destination_code) if destination_code and destination_code != "ANY" else None
    ...
    if not is_valid_search_input(origin, start_date, end_date):
        return render_template('index.html', error="All fields except destination are required")
...
def ask_ai():
    ...
    try:
        ...
        payload = {
            ...
            "messages": [
                {"role": "system", "content": "You are a travel and airline industry expert assistant."},
        ...
        if response.status_code == 200:
            ...
            for i in range(3, 0, -1):
                heading_marker = '#' * i
                answer = re.sub(f"^{heading_marker}\s+(.+)$", f"<h{i+2}>\\1</h{i+2}>", answer, flags=re.MULTILINE)
            ...
            for p in paragraphs:
                ...
                if re.search(r'^[-•]\s', p, re.MULTILINE):
                    ...
                    formatted_paragraphs.append(f"<ul class='ai-list'>{''.join(bullet_list)}</ul>")
                ...
                # Format numbered lists
                    ...
                    formatted_paragraphs.append(f"<ol class='ai-list'>{''.join(number_list)}</ol>")
